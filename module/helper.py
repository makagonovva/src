...
class Helper:
    ...
    def booksim_params(self):
        input_err = 1
        while input_err == 1:
            topology = str(input('Insert the topology: torus or mesh\n').strip().lower())
            if  topology in ['torus', 'mesh']:
                input_err = 0
            else:
                print("Error: Write torus or mesh")
                input_err = 1

        error = 1
        while error == 1:
            size = input('Insert size(for example: 4 4) \n').split()
        ...
        error = 1
        while  error == 1:
            num_vcs = input('Insert munber of virual chanels.\n')
        ...
        error = 1
        while  error == 1:
            buf_size = input('Insert size of virual chanel buffer.\n')
